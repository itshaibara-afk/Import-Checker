...
# 一覧テーブルの列見出し(小文字化・英数字以外を除去済み)から内部フィールド名への
# マッピング。実際のROVERの見出し文言をActionsログの [scrape_rover] headers:
# 出力で確認し、ここに追記/修正してください。
COLUMN_ALIASES = {
    "make": "make",
    "manufacturer": "make",
    "model": "model",
...
def normalize_header(text):
    # 空白だけでなく記号("#"や"()"など)も無視してマッチングを安定させる。
...
async def scrape_list_page(page):
    ...
    field_order = [COLUMN_ALIASES.get(h) for h in normalized]

    if not _HEADER_LOGGED:
        print(f"[scrape_rover] raw headers: {header_cells}", file=sys.stderr)
        print(f"[scrape_rover] normalized headers: {normalized}", file=sys.stderr)
        print(f"[scrape_rover] mapped fields: {field_order}", file=sys.stderr)
        unmapped = [h for h, f in zip(header_cells, field_order) if f is None]
        if unmapped:
            print(
                f"[scrape_rover] WARNING: unmapped columns (add to COLUMN_ALIASES if needed): {unmapped}",
                file=sys.stderr,
            )
        _HEADER_LOGGED = True
    ...
    for i in range(count):
        ...
        # 最初の3件だけ、デバッグ用に生の列データも残しておく
        # （Actionsログのheaders出力だけで足りない場合の保険）
        if len(results) < 3:
            record["_rawCells"] = [c.strip() for c in cells]
        if record:
...
async def go_to_next_page(page, current_page_num):
    ...
    複数の方式を順番に試し、うまくいかない場合はログに手がかりを残す。"""

    # 方式1: role="link"/"button" で "Next" 等のアクセシブルネームを持つ要素
    for name in ["Next", "Next page", "次へ", "次のページ", ">"]:
        try:
            for role in ("link", "button"):
                ...
                if n > 0:
                    first = btn.first
                    disabled = await first.get_attribute("aria-disabled")
                    classes = (await first.get_attribute("class")) or ""
                    if disabled == "true" or "disabled" in classes.lower():
                        print(f"[scrape_rover] pagination: found '{name}' but it looks disabled -> stop", file=sys.stderr)
                        return False
                    await first.click()
                    await page.wait_for_timeout(1500)
                    await page.wait_for_selector("table tbody tr", timeout=10000)
                    print(f"[scrape_rover] pagination: clicked '{name}' ({role})", file=sys.stderr)
                    return True
        except Exception:
            continue

    # 方式2: title属性やclass名に "next" を含む要素
    try:
        ...
        if n > 0:
            ...
            if "disabled" in classes.lower():
                print("[scrape_rover] pagination: title/class 'next' candidate looks disabled -> stop", file=sys.stderr)
                return False
            await first.click()
            await page.wait_for_timeout(1500)
            await page.wait_for_selector("table tbody tr", timeout=10000)
            print("[scrape_rover] pagination: clicked title/class-based 'next' candidate", file=sys.stderr)
            return True
    except Exception as e:
        print(f"[scrape_rover] pagination: title/class strategy failed: {e}", file=sys.stderr)

    # 方式3: 番号付きページネーション（現在ページ+1の数字リンクをクリック）
    try:
        ...
        if await num_link.count() > 0:
            ...
            await page.wait_for_timeout(1500)
            await page.wait_for_selector("table tbody tr", timeout=10000)
            print(f"[scrape_rover] pagination: clicked page number link '{next_num}'", file=sys.stderr)
            return True
    except Exception as e:
        print(f"[scrape_rover] pagination: numbered-link strategy failed: {e}", file=sys.stderr)

    print(
        "[scrape_rover] pagination: no working 'next page' control found. "
        "If the real register has more than one page of results, this scraper is "
        "under-collecting. Run with --headful to inspect the pagination UI and "
        "extend go_to_next_page() accordingly.",
        file=sys.stderr,
    )
    return False
...
async def run(out_path, with_details, limit, headful, max_pages):
    async with async_playwright() as p:
        ...
        while True:
            ...
            print(f"[scrape_rover] page {page_num}: {len(records)} rows", file=sys.stderr)
